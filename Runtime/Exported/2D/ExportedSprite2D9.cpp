#include "ExportedSprite2D9.h"
#include "Runtime/Render/AliceGL.h"

namespace Alice
{
	int ExportedSprite2D9::Export(lua_State*L)
	{
		luaL_Reg apis[] = {
		...
		NEW_LUA_TYPE_WITH_API(L, Sprite2D9, apis);
		SET_PARENT(L, Component2D);
		luaL_Reg api_c[] = { EXPORT_API(ClassID) { NULL,NULL } };
		EXPORT_ALICE_PACKAGE_TO_SCRIPT(L, Sprite2D9, api_c);
		return 0;
	}

	int ExportedSprite2D9::API_SetTexture(lua_State*L)
	{
		int argc = lua_gettop(L);
		if (argc == 2) {
			ExportedSprite2D9*ptr = TO_USERDATA(L, ExportedSprite2D9, 1);
			if (lua_isstring(L, 2)) {
				ptr->SetTexture("U_Texture", lua_tostring(L, 2));
			}
		}
		else if (argc == 3) {
			ExportedSprite2D9*ptr = TO_USERDATA(L, ExportedSprite2D9, 1);
	...
	int ExportedSprite2D9::API_SetCenterRect(lua_State*L){
		ExportedSprite2D9*ptr = TO_USERDATA(L, ExportedSprite2D9, 1);
	...
	int ExportedSprite2D9::API_BlendFunc(lua_State*L) {
		ExportedSprite2D9*ptr = TO_USERDATA(L, ExportedSprite2D9, 1);
		ptr->BlendFunc(lua_tointeger(L, 2), lua_tointeger(L, 3));
		return 0;
	}

	int ExportedSprite2D9::API_SetSprite(lua_State*L){
		ExportedSprite2D9*ptr = TO_USERDATA(L, ExportedSprite2D9, 1);
		if (lua_isstring(L, 2)){
	...
	int ExportedSprite2D9::API_SetMaterial(lua_State*L){
		ExportedSprite2D9*ptr = TO_USERDATA(L, ExportedSprite2D9, 1);
	...
	int ExportedSprite2D9::API_GetMaterial(lua_State*L)
	{
		ExportedSprite2D9*is = TO_USERDATA(L, ExportedSprite2D9, 1);
		is->mMaterial->PushUserData(L);
		return 1;
	}

	int ExportedSprite2D9::API_SetColor(lua_State*L)
	{
		CheckArgCount(ExportedSprite2D9::SetColor, L, 5);
		ExportedSprite2D9*ptr = TO_USERDATA(L, ExportedSprite2D9, 1);
	...
	int ExportedSprite2D9::API_SetAdditionalColor(lua_State*L)
	{
		CheckArgCount(ExportedSprite2D9::SetColor, L, 5);
		ExportedSprite2D9*ptr = TO_USERDATA(L, ExportedSprite2D9, 1);
	...
	int ExportedSprite2D9::API_SetAlpha(lua_State*L)
	{
		ExportedSprite2D9*ptr = TO_USERDATA(L, ExportedSprite2D9, 1);
	...
	int ExportedSprite2D9::API_SetSize(lua_State*L)
	{
	...
	int ExportedSprite2D9::API_GetSize(lua_State*L)
	{
		ExportedSprite2D9*ptr = TO_USERDATA(L, ExportedSprite2D9, 1);
	...
	int ExportedSprite2D9::API_UpdateTexcoord1(lua_State*L)
	{
		ExportedSprite2D9*ptr = TO_USERDATA(L, ExportedSprite2D9, 1);
