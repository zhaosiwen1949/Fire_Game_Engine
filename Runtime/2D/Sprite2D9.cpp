...
#define TexcoordMask(v) (v&_00000000_00000000_00000000_00000100)
	...
	void Sprite2D9::SetTexture(const char*uniformName, Texture2D*texture) {
	...
	void Sprite2D9::SetTexture(const char*uniformName, const char*path) {
		TextureUnitProperty*texture_unit_property = mMaterial->GetTextureProperty(uniformName);
	...
	void Sprite2D9::UpdateByTexture(Texture2D*texture) {
	...
	void Sprite2D9::SetColor(AliceUInt8 r, AliceUInt8 g, AliceUInt8 b, AliceUInt8 a){
		SetColor((float)r/ 255.0f, (float)g / 255.0f, (float)b / 255.0f, (float)a / 255.0f);
	}
	void Sprite2D9::SetColor(float r, float g, float b, float a) {
	...
	void Sprite2D9::SetAdditionalColor(float r, float g, float b, float a) {
		mAddtionalColor.Set(ALICE_MIN(r, 1.0f), ALICE_MIN(g, 1.0f), ALICE_MIN(b, 1.0f), ALICE_MIN(a, 1.0f));
		for (int i = 0; i < 16; ++i) {
			UpdateVertexTangent(i, mColor.v[0], mColor.v[1], mColor.v[2], mColor.v[3]);
		}
	}
	void Sprite2D9::SetAlpha(AliceUInt8 alpha){
		mColor.v[3] = (float)ALICE_MIN(alpha, 255) / 255.0f;
		for (int i = 0; i < 16; ++i) {
			UpdateVertexTangent(i, mColor.v[0], mColor.v[1], mColor.v[2], mColor.v[3]);
		}
	}
	void Sprite2D9::SetAtlas(std::string atlasName){
	}
	void Sprite2D9::SetOriginalSize(float width, float height){
		mOriginalWidth = width;
		mOriginalHeight = height;
	}
	void Sprite2D9::SetWidth(float value){
		if (value != mWidth) {
			mWidth = value;
			MarkGeometryDirty(mDirtyMask);
		}
	...
	void Sprite2D9::SetHeight(float value) {
		if (value != mHeight) {
			mHeight = value;
			MarkGeometryDirty(mDirtyMask);
		}
	...
	void Sprite2D9::SetSize(float w, float h) {
	...
	bool Sprite2D9::SetSpriteByName(const char*name){
	...
	void Sprite2D9::BlendFunc(GLenum src, GLenum dst) {
	}

	void Sprite2D9::SetSprite(Serializer::Sprite*sprite){
	...
	void Sprite2D9::SetSplit(float top, float right, float bottom, float left) {
		mLeftMargin = left;
		mRightMargin = right;
		mTopMargin = top;
	...
	bool Sprite2D9::SetMaterial(const char*matName)
	{
	...
	bool Sprite2D9::SetMaterial(Material*material)
	{
		mMaterial = material;
		mSpriteTexcoordU.Set(0, 1);
		mSpriteTexcoordV.Set(0, 1);
	...
	void Sprite2D9::UpdateVertexData() {
	...
	void Sprite2D9::UpdateTexcoordData(){
	...
	void Sprite2D9::UpdateBoundingBox(){
	...
	void Sprite2D9::Update(float deltaTime){
	...
	void Sprite2D9::Render(RenderQueue*rq
#if ALICE_EDITOR
		, DrawCallInfo &rs
#endif
	)
	{
		...
		GetRect(sprite_rect);
		if (false == rq->mCanvas->mRect.IsOverlappedWith(sprite_rect)) {
			return;
		}
	...
	void Sprite2D9::UpdateVertexNormal(int nIndex, float x, float y, float z, float w /* = 1.0f */){
		VertexDataFull*vertexes = mVertexData.GetBuffer<VertexDataFull>();
		vertexes[nIndex].mNormal.Set(x, y, z, w);
		MarkColorDirty(mDirtyMask);
	}

	void Sprite2D9::UpdateVertexTangent(int nIndex, float x, float y, float z, float w /* = 1.0f */){
		VertexDataFull*vertexes = mVertexData.GetBuffer<VertexDataFull>();
		vertexes[nIndex].mTangent.Set(x*mAddtionalColor.v[0], y*mAddtionalColor.v[1], z*mAddtionalColor.v[2], w*mAddtionalColor.v[3]);
		MarkColorDirty(mDirtyMask);
	}

	void Sprite2D9::UpdateVertexTexcoord1(int nIndex, float x, float y, float z, float w /* = 1.0f */){
		VertexDataFull*vertexes = mVertexData.GetBuffer<VertexDataFull>();
